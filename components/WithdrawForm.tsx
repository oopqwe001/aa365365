  ...
  return (
    <div className="p-4 bg-white min-h-screen">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-base font-black">{t('finance.withdraw_title')}</h2>
      </div>
      ...
      <div className="bg-orange-50 p-3 rounded-lg mb-6 border border-orange-100">
        <p className="text-[10px] text-orange-700 font-bold leading-tight">
          {t('finance.withdraw_info', { defaultValue: '【重要】出金には審査が必要です。通常1〜2営業日以内に完了します。口座名義が一致しない場合、承認されませんのでご注意ください。' })}
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-[10px] text-gray-400 mb-1">{t('finance.amount')} (¥)</label>
          <input 
            type="number" 
        ...
        <div className="bg-gray-50 p-4 rounded-xl space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] text-gray-400 mb-1">{t('finance.bank_name')}</label>
            ...
            <div>
              <label className="block text-[10px] text-gray-400 mb-1">{t('finance.branch_name')}</label>
              <input type="text" className="w-full bg-white border border-gray-200 rounded p-2 text-xs" required
                value={formData.branchName} onChange={e => setFormData({...formData, branchName: e.target.value})} />
            </div>
          </div>
          <div>
            <label className="block text-[10px] text-gray-400 mb-1">{t('finance.account_number')}</label>
            <input type="text" className="w-full bg-white border border-gray-200 rounded p-2 text-xs" required
              value={formData.accountNumber} onChange={e => setFormData({...formData, accountNumber: e.target.value})} />
          </div>
          <div>
            <label className="block text-[10px] text-gray-400 mb-1">{t('finance.account_name')}</label>
            <input type="text" className="w-full bg-white border border-gray-200 rounded p-2 text-xs" placeholder={t('finance.account_name_placeholder', { defaultValue: 'ヤマダ タロウ' })} required
