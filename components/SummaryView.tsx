      ...
      <div className="p-3 flex-1 overflow-y-auto">
        <div className="flex justify-between items-center mb-3">
          <div>
            ...
            <p className="text-[9px] text-gray-400">{t('summary.draw_time')}</p>
          </div>
          <button onClick={onBack} className="text-[10px] text-blue-600 font-bold">{t('summary.back')}</button>
        </div>
                  ...
                  {sel.numbers.length > 0 ? (
                    <>
                      ...
                      <div className="flex gap-3 text-[9px] text-gray-400 font-bold">
                        <span>{t('summary.quantity', { count: 1 })}</span>
                        <span>{t('summary.amount', { amount: game.price })}</span>
                      </div>
                    </>
                  ) : (
