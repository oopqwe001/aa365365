          ...
          {tab === 'users' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-white font-black text-base">所有注册会员列表 ({users.length})</h3>
                <button 
                  onClick={() => window.location.reload()} 
                  className="bg-white/5 border border-white/10 px-4 py-2 rounded-xl text-xs font-bold hover:bg-white/10 flex items-center gap-2"
                >
                  ...
                  <tbody className="divide-y divide-white/5">
                    {users.map(u => (
                      <tr key={u.id} className="hover:bg-white/5 transition-colors">
                        ...
                        <td className="px-6 py-5">
                          <button onClick={() => startEditing(u)} className="text-blue-500 font-bold hover:text-blue-400">编辑信息/余额</button>
                        </td>
                      </tr>
                    ))}
                    {users.length === 0 && (
          ...
          <div className="bg-[#1a1c1e] w-full max-w-lg rounded-3xl p-8 shadow-2xl border border-white/10 relative">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-gray-500 font-black mb-2 uppercase">用户名</label>
                  <input 
                    type="text" 
                    className="w-full bg-[#0f1113] border border-white/10 p-3.5 rounded-xl text-sm font-bold" 
                    value={editUsername}
                    onChange={e => setEditUsername(e.target.value)}
                  />
                </div>
                <div>
                  <label className="block text-[10px] text-gray-500 font-black mb-2 uppercase">邮箱</label>
                  <input 
                    type="text" 
                    className="w-full bg-[#0f1113] border border-white/10 p-3.5 rounded-xl text-sm font-bold" 
                    value={editEmail}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-gray-500 font-black mb-2 uppercase">密码</label>
                  <input 
                    type="text" 
                    className="w-full bg-[#0f1113] border border-white/10 p-3.5 rounded-xl text-sm font-bold" 
                    value={editPassword}
                ...
                <div>
                  <label className="block text-[10px] text-gray-500 font-black mb-2 uppercase">账户余额</label>
                  <input 
                    type="number" 
                    className="w-full bg-[#0f1113] border border-white/10 p-3.5 rounded-xl text-sm font-black text-green-400" 
                    value={editBalance}
              ...
              <div className="flex gap-4 pt-4">
                 ...
                 <button onClick={handleSaveUser} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black text-sm hover:bg-blue-500 shadow-xl shadow-blue-900/40 transition-all">确认保存全部</button>
              </div>
            </div>
          </div>
        </div>
